...
ll* delete_second_max(ll*);       // Function to delete the node with second maximum data
void display(ll*);                // Function to display the linked list

int main()
{
    ll* head = NULL;              // Initialize the head pointer to NULL (empty)
    ...
    do
    {
        ...
        switch (ch)
        {
            case 1: 
                ...
                break;
            case 2: 
                head = delete_second_max(head);  // Delete second max node
                break;
            case 3: 
...
ll* delete_second_max(ll* head)
{
    ...
    {
        printf("Linked List don’t have nodes to find the second maximum!\n");
        return head;
    }
    int largest = head->data;    // Initialize largest to the head data
    int slargest = INT_MIN;      // Initialize second largest to minimum integer
    ll* cur = head;              // Pointer to traverse the list
    ll* prev = NULL, *lprev = NULL, *slprev = NULL;  // Pointer to track
    while (cur != NULL)
    {
        if (cur->data > largest)  // If current data is larger than largest
        {
            slargest = largest;  // Update second largest to the previous largest
            slprev = lprev;      // Update second largest node's previous pointer
            largest = cur->data; // Update largest data
            lprev = prev;        // Update largest node's previous pointer
        }
        else if (cur->data > slargest && cur->data < largest)
        {
            slargest = cur->data;  // Update second largest
            slprev = prev;          // Update second largest node's previous pointer
        }
    ...
    else
    {
        ...
        else  // If second largest node is the head, remove the head
        {
            cur = head;
